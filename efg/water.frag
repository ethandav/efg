...
float4 main(Params input) : SV_Target
{
    ...
    float depth = length(input.FragPos - viewPos);
    float transparency = clamp(1.0 - exp(-0.05 * depth), 0.2, 1.0);  // Adjust parameters as needed

    // Apply Fresnel effect for surface reflectivity
